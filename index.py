...
with tabs[0]:
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            systolic_bp = st.number_input("Systolic Blood Pressure", 80, 200, 110)
            diastolic_bp = st.number_input("Diastolic Blood Pressure", 40, 120, 70)
            gestational_age_weeks = st.number_input("Gestational Age (weeks)", 24, 42, 38)
            hemoglobin = st.number_input("Hemoglobin (g/dL)", 5.0, 18.0, 11.0)
        with col2:
            parity = st.selectbox("Parity", [0, 1, 2, 3, 4, "5+"])
            alcohol = st.radio("Alcohol Consumption", ["Yes", "No"], horizontal=True)
            education_level = st.selectbox("Education Level", ["None", "Primary", "Secondary", "Higher"])
